    ...
    try {
        ...
        try {
            ...
            if (!lastMessage || !lastMessage.content) {
                return new Response(JSON.stringify({ error: "Last message must have content" }), { status: 400 });
            }
            ...
            try {
                ...
                if (!Array.isArray(existingThread) || existingThread.length === 0) {
                    ...
                    const title = firstMessageContent.substring(0, 50) || "New Chat";
                    await query('INSERT INTO threads (id, title) VALUES (?, ?)', [threadId, title]);
                }
            } catch (dbError) {
            ...
            try {
                await query('INSERT INTO messages (id, thread_id, role, content) VALUES (?, ?, ?, ?)', [
                    uuidv4(),
                    threadId,
                    lastMessage.role || 'user', // Default to user if undefined
                    lastMessage.content // Validated above
                ]);
            } catch (dbError) {
