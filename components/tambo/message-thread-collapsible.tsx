...
const CollapsibleContainer = React.forwardRef<
  HTMLDivElement,
  { isOpen: boolean; onOpenChange: (open: boolean) => void; children: React.ReactNode } & React.HTMLAttributes<HTMLDivElement>
      ...
      <div className="relative flex items-center justify-center w-full h-full">
        <Collapsible.Trigger asChild>
          <button
            ...
            tabIndex={0}
          >
            <Image src="/logo/icon/Octo-Icon.svg" width={32} height={32} alt="Octo Icon" className="w-8 h-8" />
          </button>
        </Collapsible.Trigger>
      </div>
    );
    ...
    <div className="w-full">
      <div className="flex items-center justify-between w-full p-4 border-b border-gray-200 shadow-sm">
        <div className="flex items-center space-x-2">
          <Image src="/logo/icon/Octo-Icon.svg" width={24} height={24} alt="Octo Icon" className="w-4 h-4" />
          <span>{config.labels.openState}</span>
        </div>
        <div role="button" className="p-1 rounded-full hover:bg-muted/70 transition-colors cursor-pointer" onClick={onClose} aria-label="Close">
...
export const MessageThreadCollapsible = React.forwardRef<HTMLDivElement, MessageThreadCollapsibleProps>(
  ({ className, contextKey, defaultOpen = false, initialQuery, variant, ...props }, ref) => {
    const searchParams = useSearchParams();
    const queryFromUrl = searchParams.get("q") || undefined;
    const finalInitialQuery = initialQuery || queryFromUrl;

    const [isOpen, setIsOpen] = React.useState(defaultOpen || !!finalInitialQuery);
    ...
    return (
      <CollapsibleContainer ref={ref} className={className} isOpen={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger isOpen={isOpen} onClose={() => setIsOpen(false)} onThreadChange={handleThreadChange} config={THREAD_CONFIG} />
        {/* Full panel content */}
        <div id="message-thread-content" className="flex flex-col h-full">
